...
class Api {
  ...
  Future<List<Movie>> getTregindMovies(List<Genero> generos, String generosSelect,) async {
    String trendingUrl = 'https://api.themoviedb.org/3/discover/movie?&include_adult=false&with_genres=$generosSelect&include_video=false&language=pt-br&page=1&sort_by=popularity.desc&api_key=${Constants.apiKey}';
    //String _detalhesMovie = 'https://api.themoviedb.org/3/movie/${movideId}?api_key=${Constants.apiKey}';

    final response = await http.get(Uri.parse(trendingUrl)); // conexão
